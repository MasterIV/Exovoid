...
export function calculateWeaponActions(
  weapon: WeaponType,
  talents: string[] = [],
) {
  ...
  if (weapon.qualities["Burst"]) {
    const penalty = weapon.qualities["Compensated"]
      ? Math.max(0, 2 - weapon.qualities["Compensated"])
      : 2;
    const ammo = talents.includes("Recoil Control")
      ? Math.ceil(weapon.qualities["Burst"] * 0.6)
      : weapon.qualities["Burst"];
    actions.burst = {
      id: "burst",
      name: "Fire Burst",
      ap: weapon.speed,
  ...
  if (weapon.qualities["Full Auto"]) {
    const penalty = weapon.qualities["Compensated"]
      ? Math.max(0, 3 - weapon.qualities["Compensated"])
      : 4;
    const ammo = talents.includes("Recoil Control")
      ? Math.ceil(weapon.qualities["Full Auto"] * 0.6)
      : weapon.qualities["Full Auto"];
    actions["auto-main"] = {
      id: "auto-main",
      name: "Full Auto Main Attack",
      ap: weapon.speed * 2,
      skill: weapon.skill,
      modifier: modifier - penalty,
      ammo,
    ...
    if (talents.includes("Dual Wield")) {
      actions["dual-auto-main"] = {
        id: "dual-auto-main",
        name: "Dual Wield Full Auto Main Attack",
        ap: Math.floor(weapon.speed / 2) * 2,
        skill: weapon.skill,
        modifier: modifier - penalty,
        ammo,
      };
...
export function calculateCombatActions(stats: CharacterType) {
  const actions: Record<string, CombatAction> = {
    ...
    secondWind: {
      id: "secondWind",
      name: "Second Wind",
      ap: stats.talents.includes("Third Wind") ? 2 : 4,
      skill: "Survival",
    },
    ...
    firstAid: {
      id: "firstAid",
      name: "First Aid",
      ap: stats.talents.includes("Field Doctor") ? 5 : 8,
      skill: "Medicine",
    },
  };
