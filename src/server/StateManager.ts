...
type OverlappingEvents = Extract<
  EventNames<ServerEvents>,
  EventNames<ClientEvents>
>;
...
export default class StateManager {
  ...
  registerAuthenticationHandlers(socket: ClientSocket) {
    ...
    socket.on(
      "login",
      this.wrapHandler(socket, (name, password) => {
    ...
    socket.on(
      "relogin",
      this.wrapHandler(socket, (name, token) => {
  ...
  registerTableSelectionHandlers(socket: ClientSocket) {
    ...
    socket.on(
      "create",
      this.wrapHandler(socket, (name, table, password) => {
        if (!socket.data.account) throw new Error("Login required!");
  ...
  registerGameHandlers(socket: ClientSocket) {
    ...
    socket.on(
      "roll",
      this.wrapHandler(socket, (pool, metadata = {}) => {
