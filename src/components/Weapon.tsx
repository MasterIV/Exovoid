...
function createChips(data: Record<string, number | undefined>) {
  return (
    <Stack direction="row" spacing={1} marginX={1} display={"inline-block"}>
...
export default React.memo(function Weapon({
  locked,
  heft,
  onChange,
  onRemove,
  ...
  const details = useMemo(
    () => applyWeaponMods(weapon),
    [weapon.type, weapon.mods],
  );
  ...
  const changeAmmo = (k: string, v: number) =>
    onChange("ammo", { ...weapon.ammo, [k]: v });
  const performAction = () => onAction(actions[action], weapon);

  const removeWeapon = (e: any) => {
    e.stopPropagation();
    if (window.confirm("Remove Weapon?")) onRemove();
  };
  ...
  if (details.skill === "Melee") damage += details.hands * heft;

  return (
    <Accordion
      expanded={Boolean(weapon.expanded)}
      ...
      <AccordionDetails>
        <Grid container direction="column" spacing={2}>
          ...
          >
            ...
            <Grid item xs={4}>
              <Btn fullWidth onClick={performAction}>
                Execute
              </Btn>
            </Grid>
          </Grid>
