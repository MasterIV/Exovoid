...
export function DiceSymbol({ type, symbols, exploded }: DiceSymbolProps) {
  return (
    <div
      style={{ backgroundImage: `url("/img/dice/${type}.png")` }}
      className={`dice large ${exploded && "exploded"}`}
    >
      {symbols.length > 0 && (
        <img
          alt={symbols.join("_")}
          src={`/img/symbols/${symbols.join("_")}.png`}
          width={24}
        />
      )}
    </div>
...
export function DiceCounter({ type, count, large = false }: DiceCounterProps) {
  return (
    <div
      style={{ backgroundImage: `url("/img/dice/${type}.png")` }}
      className={large ? `dice large` : `dice small`}
    >
      {count}
    </div>
  );
  ...
  return (
    <div className={"dice-pool"}>
      {Object.keys(dice).map((d) => {
        ...
        return props[key] ? (
          <DiceCounter key={d} large={large} type={d} count={props[key] || 0} />
        ) : null;
      })}
    </div>
  );
