...
allInjuries.forEach((i) => {
  injuryMap[i.name] = i;
  if (!injuryBuckets[i.severity]) injuryBuckets[i.severity] = [];
...
function Injury({
  ...
  return (
    <Grid item xs={npc ? 12 : "auto"}>
      <Paper className="injury" style={{ minHeight: 40, background: "#311" }}>
        <Btn
          onClick={onRemove}
          color="error"
...
const rollDefaults: InjuryRoll = {
  show: false,
  damage: 1,
  modifier: 0,
  injury: allInjuries[0].name,
};
...
export default function Injuries({
  ...
  const id = uuid.v4();
  const mapped = injuries.map((i: string, k: number) => ({
    ...injuryMap[i],
    id: i + k,
  }));
  const diceCount =
  ...
  const rollCallback = (result: DiceResultType, metadata: any) => {
    ...
    if (severity > 0) {
      ...
      changeInjuries([
        ...injuries,
        bucket[(Math.random() * bucket.length) | 0].name,
      ]);
    }
  ...
  const takeDamage = () => {
    if (damage) changeHealth(health - damage);
  ...
  return (
    <>
      <Collection
        ...
        values={mapped}
        onChange={(data) =>
          changeInjuries(data.map((i) => (i as InjuryType).name))
        }
        component={Injury}
      />
      ...
      <Modal open={roll.show} onClose={resetRoll}>
        <Paper className="paperSmall">
          <Stack spacing={3} textAlign="center">
            ...
            {damage > health && diceCount > 0 && (
              <DicePool injury={diceCount} large />
            )}
      ...
      <Grid item>
        <Btn onClick={() => changeRoll("show", true)} fullWidth>
          Take Damage
        </Btn>
      </Grid>
    </>
