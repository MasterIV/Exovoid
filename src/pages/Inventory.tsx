...
function Item({ onChange, onRemove, locked, ...item }: ItemProps) {
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        <TextInput name="location" values={item} onChange={onChange} />
      </TableCell>
      <TableCell>
        <Stack direction="row" spacing={2}>
          <TextInput name="note" values={item} onChange={onChange} />
          <Btn disabled={locked} color="error" onClick={removeItem}>
            Remove
          </Btn>
        </Stack>
      </TableCell>
...
export default React.memo(function InventoryPage({
  ...
  const changeInventory = useCallback(
    (i: any) => onChange("inventory", i),
    [onChange],
  );
  const changeCurrency = (k: string, v: number) =>
    onChange("currency", { ...currency, [k]: v });
    ...
    <Grid container spacing={2} margin={1} direction="column">
      ...
      <Grid item container spacing={2} alignItems="center">
        <Grid xs={8} item>
          <Btn
            onClick={() =>
              changeInventory([...inventory, { ...defaults, id: uuid.v4() }])
            }
          >
            Add Item
          </Btn>
