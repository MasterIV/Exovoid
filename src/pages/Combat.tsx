...
import {
  ...
  CombatAction,
  formatAction,
} from "../logic/calculateCombatActions";
...
export default function CombatPage({
  ...
  const changeWeapons = useCallback(
    (data: any) => onChange("weapons", data),
    [onChange],
  );
  const changeData = useCallback(
    (k: string, v: any) => setData((old) => ({ ...old, [k]: v })),
    [],
  );
  const joinCombat = () => socket.emit("combatant", charToCombatant(stats));
  const leaveCombat = () =>
    window.confirm("Leave Combat?") && socket.emit("remove", stats.id);
  ...
  const changeArmor = useCallback(
    (data: any) => onChange("armor", data),
    [onChange],
  );
  const addArmor = () =>
    onChange("armor", [
  ...
  const performAction = (
    action: CombatAction,
    weapon: CharacterWeapon | null = null,
  ) => {
    if (
      action.id !== "reload" &&
      Number(action.ammo) > Number(weapon?.ammo.loaded)
    )
    ...
    if (action.id === "reload" && action.ammo && weapon) {
      const loaded = Math.min(
        action.ammo,
        weapon.ammo.loaded + weapon.ammo.reserve,
      );
      const reserve = weapon.ammo.reserve - loaded + weapon.ammo.loaded;
      onChange(
        "weapons",
        characterWeapons.map((w) =>
          w.id === weapon.id ? { ...w, ammo: { loaded, reserve } } : w,
        ),
      );
    }
  ...
  const changeHealth = useCallback(
    (hp: number) => onChange("currentHealth", hp),
    [onChange],
  );
  const changeInjuries = useCallback(
    (i: string[]) => onChange("injuries", i),
    [onChange],
  );
  ...
  return (
    <Grid container spacing={2} margin={1}>
      <Grid item xs={3}>
        <Initiative>
          <Grid item>
            <Btn fullWidth onClick={joinCombat}>
              Join Combat
            </Btn>
          </Grid>

          <Grid item>
            <Btn fullWidth onClick={leaveCombat} color="error">
              Leave Combat
            </Btn>
          </Grid>
          ...
          <Grid item>
            <Btn fullWidth onClick={() => performAction(actions[data.action])}>
              Execute
            </Btn>
          </Grid>
        </Initiative>
      ...
      <Grid item xs={6}>
          ...
          <Grid item xs={8}>
            <Dropdown
              ...
              options={weapons.map((w) => ({
                id: w.weapon,
                name: `${w.name} (${w.weapon})`,
              }))}
            />
          </Grid>
          <Grid item xs={4}>
            <Btn fullWidth size="large" onClick={addWeapon}>
              Add Weapon
            </Btn>
          </Grid>
        </Grid>
        ...
        >
          ...
          <Grid item xs={4}>
            <Btn fullWidth size="large" onClick={addArmor}>
              Add Armor
            </Btn>
          </Grid>
        </Grid>
      ...
      <Grid container direction="column" item xs={3} spacing={2}>
        ...
        <Grid item textAlign="center">
          <Value
            ...
            width={250}
            label="Edge"
            mask={` / ${calculateEdge(stats)}`}
            value={stats.currentEdge}
            onChange={onChange}
          />
        </Grid>
