...
export default React.memo(function CharacterPage({
  stats,
  onChange,
  onRoll,
  locked = false,
  ...
  const changeSkill = useCallback(
    (name: string, value: number) =>
      onChange("skills", { ...skills, [name]: value }),
    [onChange, skills],
  );
        ...
        <Grid item xs={7}>
          ...
          <Paper style={derivedStyles}>
            <Typography align={"center"}>
              Action Points {calculateActionPoints(stats)}, Speed{" "}
              {calculateSpeed(stats)}, Heft {calculateHeft(stats)},
        ...
        <Grid item container xs={2} spacing={1} direction="column">
          ...
          <Grid item textAlign="right">
            <Btn
              fullWidth
              className="roll-btn"
              onClick={() => onRoll(vigilance, 0, 0, { skill: "Vigilance" })}
            >
              Vigilance
