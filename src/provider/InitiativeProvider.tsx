...
function updateCombatant(updated: Combatant) {
  ...
  timers[updated.id] = window.setTimeout(
    () => socket.emit("combatant", updated),
    UPDATE_DELAY,
  );
}
...
export default function InitiativeProvider({
  children,
  stats,
}: InitiativeProviderProps) {
  ...
  useEffect(() => {
    socket.on("combatant", (data) =>
      setCombatants((old) => ({ ...old, [data.id]: data })),
    );
  ...
  useEffect(() => {
    if (
      combatants[stats.id] &&
      combatants[stats.id].currentHealth !== stats.currentHealth
    )
      update(charToCombatant(stats, combatants[stats.id].currentAp));
    stats.npcs?.forEach((npc) => {
      if (
        combatants[npc.id] &&
        combatants[npc.id].currentHealth !== npc.currentHealth
      )
  ...
  return (
    <InitiativeContext.Provider
      value={{ combatants, reset, round, update, spendAp }}
    >
      {children}
    </InitiativeContext.Provider>
  );
