...
function Game({ character, error, onChange }: GameProps) {
    ...
    if (roll.metadata?.weapon && roll.metadata?.ammo)
      onChange(
        ...
        character.weapons.map((w) => {
          const ammo = {
            ...w.ammo,
            loaded: w.ammo.loaded - roll.metadata?.ammo,
          };
  ...
  const changeNpc = React.useCallback(
    (npcs: object[]) => onChange("npcs", npcs),
    [onChange],
  );
  ...
  const tabs = [
    ...
    },
    {
      name: "Talents",
      content: () => (
        <TalentPage locked={locked} stats={character} onChange={onChange} />
      ),
    },
    {
      name: "Cyberware",
      content: () => (
        <CyberWarePage locked={locked} stats={character} onChange={onChange} />
      ),
    },
    {
      name: "Inventory",
      ...
      <Modal open={roll.show} onClose={resetRoll}>
        <Paper className="paperSmall">
          <Stack spacing={3} textAlign="center">
            ...
            />
            <DicePool {...pool} large />
            <Btn disabled={emptyPool(pool)} onClick={onRoll}>
              Roll!
            </Btn>
          </Stack>
        </Paper>
